...
void FWApplication::DrawTexture(SDL_Texture * texture, int xOffset, int yOffset, int width, int height, bool center)
{
...
Rect FWApplication::DrawText(const std::string & message, uint32_t offsetX, uint32_t offsetY, Color pColor, bool center)
{
	Rect size{ offsetX, offsetY, 0,0 };
	SDL_Color color = { pColor.r, pColor.g, pColor.b, pColor.a };
	...
	if (surface)
	{
		...
		if (texture)
		{
			...
			DrawTexture(texture, offsetX, offsetY, surface->w, surface->h, center);
...
void FWApplication::SetWindowSize(int width, int height)
{
